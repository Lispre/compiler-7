...
;
components: components component		{$<tn>$ = ast->addToLastRight($<tn>1, ast->createNode($<tn>2, 0, CompNode));cout<<"components: components component\n";}
			|component					{$<tn>$ = ast->createNode($<tn>1, 0, CompNode);cout<<"components: component\n";}
;
component:	class_interface				{$<tn>$ = $<tn>1;cout<<"component:	class_interface\n";}
...
s->insert_scope($<r.str>1,s->currScope);s->currScope=s->currScope->parent;cout<<"class_interface: class_interface_header class_interface_body\n";}
; 
	...
	type IDENTIFIER	SEMI_COMA									
			{
				...
				if($<r.type>1==6)
					if(s->insertVariableInCurrentScope($<r.str>2,$<r.str>1,visability) == 0)	Er->errQ->enqueue(yylval.r.myLineNo,yylval.r.myColno,"ERROR","Variable redefine");
				else
...
;
complex_type:	IDENTIFIER	MULTI					{$<r.type>$=6;$<r.str>$=$<r.str>1;if(s->check_var_type($<r.str>1)==0)	Er->errQ->enqueue(yylval.r.myLineNo,yylval.r.myColno,"ERROR","type is undefined");cout<<"complex_type:	IDENTIFIER	MULTI\n";}
;
...
class_implementation:
	class_implementation_header class_implementation_body		{$<tn>$=ast->createNode($<tn>1,$<tn>2,ImpNode);
										$<tn>$->currST=s->currScope;
										s->insert_scope1($<r.str>1,s->currScope); s->currScope=s->currScope->parent;cout<<"class_implementation: class_implementation_header class_implementation_body\n";}
;
class_implementation_header: 
	AT_IMPLEMENTATION IDENTIFIER SEMI_COLUMN IDENTIFIER			{$<tn>$=ast->createNode($<tn>1,$<tn>2,HedrImpNode);Interface_name=$<r.str>2;i=2;$<r.str>$=$<r.str>2;if(s->check_Implementation_Interface($<r.str>2)==0) Er->errQ->enqueue(yylval.r.myLineNo,yylval.r.myColno,"ERROR","Implementation has no Interface");
...
for_loop_header:
	...
	|FOR OPEN_P	for_initializer	SEMI_COMA			 SEMI_COMA				CLOSE_P	
										{$<tn>$=ast->createNode($<tn>3,0,ForHdrNode);	cout<<"for_loop_header: FOR OPEN_P	for_initializer	SEMI_COMA			 SEMI_COMA				CLOSE_P	\n";}
	|FOR OPEN_P					SEMI_COMA logic_expr SEMI_COMA				CLOSE_P	
										{$<tn>$=ast->createNode($<tn>4,0,ForHdrNode);	cout<<"for_loop_header: FOR OPEN_P					SEMI_COMA logic_expr SEMI_COMA				CLOSE_P\n";}
...
simple_expr:
	...
	|p_type simple_expr				%prec p_type_expr_prec	{/* aaaaa*/ cout<<"expr:p_type expr\n";}//casting
;
block_body:
...
conditional_statement:
	...
	|if_header statement	ELSE 	statement	{$<tn>$=ast->createNode($<tn>1,$<tn>2,CondtiNode);
...
;
protocol: protocol_header protocol_body		{$<tn>$ = ast->createNode($<tn>1,$<tn>2,ProtclNode);
				s->insert_scope1($<r.str>1,s->currScope);s->currScope=s->currScope->parent;cout<<"protocol: protocol_header protocol_body\n";}
;
protocol_header:
		AT_PROTOCOL IDENTIFIER	{$<tn>$ = ast->createNode(0,0,ProtclHdrNode);
								$<r.str>$=$<r.str>2;
